...
$$P(t) = e^{i\varphi} \exp(i\theta \hat{a}^{\dagger}\hat{a})\exp{\left(\frac{\tanh{r}}{2}\hat{a}^{\dagger 2}\right)}\times:\frac{\exp{([1/\cosh{r}-1]\hat{a}^{\dagger}\hat{a})}}{\sqrt{\cosh{r}}}:\times\exp{\left(-\frac{\tanh{r}}{2}\hat{a}^{2}\right)}\exp{(i(\pi/2-\theta)\hat{a}^{\dagger}\hat{a})}.$$
